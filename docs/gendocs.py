...
Modified version of https://github.com/drckf/paysage/blob/master/docs/gendocs.py
"""
...
if __name__ == "__main__":
    import natural_bm
    savedocs(walk_through_package(natural_bm))
    savedocs(walk_through_package(natural_bm.backend))
    savedocs(walk_through_package(natural_bm.datasets))
    
    # Cleanup some unneded files
    junk_ls = ['json', 'np', 'os', 'sys', 'T', 'theano']
    path = os.path.abspath(__file__)
    folder = '/'.join(path.split('/')[:-1])
    for junk in junk_ls:
        try:
            os.remove(folder+'/'+junk+'.md')
        except OSError:
            pass
